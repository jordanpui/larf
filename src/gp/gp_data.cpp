...
void gp_copy_in(vector<Group> &groups, bool layout){
    if(layout) gp_copy_group_in(groups);
    else gp_copy_place_in(groups);
}
...
void gp_copy_group_in(vector<Group> &groups){
    ...
    for(auto master : database.masters){
        if(gs.packed){
            switch (master->name) {
                ...
                case Master::LUT2 :
                case Master::LUT1 :
                case Master::CARRY8 :   MasterArea[master] = 1.0 * areaScale; break;
                case Master::DSP48E2 :  MasterArea[master] = gs.dspArea; break;
                case Master::RAMB36E2 : MasterArea[master] = gs.ramArea; break;
                case Master::BUFGCE : 
                case Master::IBUF :
                case Master::OBUF :     MasterArea[master] = 1.0; break;
                default :               printlog(LOG_WARN, "unknown master name: %s", master->name);
                                        MasterArea[master] = 1.0; break;
            }
        }else{
            switch (master->name) {
                case Master::FDRE :     MasterArea[master] = gs.ffArea * areaScale; break;
                case Master::LUT6 :     MasterArea[master] = 2 * gs.lutArea * areaScale; break;
                case Master::LUT5 :
                case Master::LUT4 :
                case Master::LUT3 :
                case Master::LUT2 :
                case Master::LUT1 :     MasterArea[master] = gs.lutArea * areaScale; break;
                case Master::CARRY8 :   MasterArea[master] = 0.1 * areaScale; break;
                case Master::DSP48E2 :  MasterArea[master] = gs.dspArea; break;
                case Master::RAMB36E2 : MasterArea[master] = gs.ramArea; break;
                case Master::BUFGCE :
                case Master::IBUF :
                case Master::OBUF :     MasterArea[master] = 0.1; break;
                default :               printlog(LOG_WARN, "unknown master name: %s", master->name);
                                        MasterArea[master] = 0.1; break;
            }
        }
    ...
    for(size_t i=0; i<database.nets.size(); i++){
        ...
        if(gs.degreeNetWeight){
            if(nPins < 10)          netWeight[i] = 1.06;
            else if(nPins < 20)     netWeight[i] = 1.2;
            else if(nPins < 30)     netWeight[i] = 1.4;
            else if(nPins < 50)     netWeight[i] = 1.6;
            else if(nPins < 100)    netWeight[i] = 1.8;
            else if(nPins < 200)    netWeight[i] = 2.1;
            else                    netWeight[i] = 3.0;
        }else{
            netWeight[i] = 1.0;
        }
    }
    ...
    for(int i=0; i<numFences; i++){
        if(fenceCellArea[i] > fenceFreeArea[i] * 0.95){
            ...
            if (fenceCellArea[i] != 0) printlog(LOG_INFO, "cell area OK for fence %d : %lf vs %lf", i, fenceCellArea[i], fenceFreeArea[i]);
        }
    }
    ...
    for(int i=0; i<(int)regions.size();i++){
        printlog(LOG_INFO,"region id:%d sitetype->id:%d site type:%s",i,regions[i].type->id,regions[i].type->name.c_str());
    }
    */
}
...
FRegion::FRegion(SiteType *type){
    this->type = type;
}
void FRegion::addRect(FRect rect){
