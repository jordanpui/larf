...
void DisjGraph::DivideByRegions(vector<vector<int>> &pointSets, vector<Point2d> &poss, unsigned maxSize) {
    ...
    for (unsigned i = 0; i < pointSets.size(); ++i)
        for (auto v : pointSets[i]) bounds[i].update(poss[v].y, poss[v].x);
    while (tooMany) {
        tooMany = false;
        for (unsigned i = 0; i < pointSets.size(); ++i) {
            if (pointSets[i].size() > maxSize) {
                ...
                if ((bounds[i].y()) > (bounds[i].x())) {  // horizontally divide
                    double boundary = bounds[i].cy();
                    for (auto v : tempSet) {
                        if (poss[v].y > boundary)
                            pointSets[i].push_back(v);
                        else
                            pointSets.back().push_back(v);
                    }
                    bounds.push_back(bounds[i]);
                    bounds.back().uy() = boundary;
                    bounds[i].ly() = boundary;
                } else {  // vertically divide
                    double boundary = bounds[i].cx();
                    for (auto v : tempSet) {
                        if (poss[v].x > boundary)
                            pointSets[i].push_back(v);
                        else
                            pointSets.back().push_back(v);
                    }
    ...
    //    printlog(LOG_INFO, "\tsize:%d, lo:%lf, up:%lf, le:%lf, ri:%lf", (int)graphs[i].vertices.size(),
    //    bounds[i].lower, bounds[i].upper, bounds[i].left, bounds[i].right);
}
...
int GenGraph::GetEdgeDelta(const Edge &e) { return lab[e.v] + lab[e.u] - mat[e.v][e.u].w * 2; }

void GenGraph::UpdateSlackV(int v, int x) {
    if (!slackv[x] || GetEdgeDelta(mat[v][x]) < GetEdgeDelta(mat[slackv[x]][x])) slackv[x] = v;
}

void GenGraph::CalcSlackV(int x) {
    slackv[x] = 0;
    for (int v = 1; v <= n; v++)
        if (mat[v][x].w > 0 && bel[v] != x && col[bel[v]] == 0) UpdateSlackV(v, x);
}
...
void GenGraph::QPush(int x) {
    if (x <= n)
        ...
        for (int i = 0; i < (int)bloch[x].size(); i++) QPush(bloch[x][i]);
    }
}
...
void GenGraph::SetMate(int xv, int xu) {
    ...
    if (xv > n) {
        ...
        for (int i = 0; i < pr; i++) SetMate(bloch[xv][i], bloch[xv][i ^ 1]);
        SetMate(xr, xu);
...
void GenGraph::SetBel(int x, int b) {
    bel[x] = b;
    if (x > n) {
        for (int i = 0; i < (int)bloch[x].size(); i++) SetBel(bloch[x][i], b);
    }
}
...
void GenGraph::ExpandBlossom(int b) {
    for (int i = 0; i < (int)bloch[b].size(); i++) SetBel(bloch[b][i], bloch[b][i]);
    ...
    if (col[xu] == -1) {
        ...
        if (!xa) {
            ...
            for (int b = n + 1; b <= n_x; b++)
                if (bel[b] == b && lab[b] == 0) ExpandBlossomFinal(b);
            return true;
        } else
            AddBlossom(xv, xa, xu);
    }
...
bool GenGraph::DoMatching() {
    ...
    q_n = 0;
    for (int x = 1; x <= n_x; x++)
        if (bel[x] == x && !mate[x]) fa[x] = 0, col[x] = 0, slackv[x] = 0, QPush(x);
    ...
    while (true) {
        for (int i = 0; i < q_n; i++) {
            ...
            for (int u = 1; u <= n; u++)
                if (mat[v][u].w > 0 && bel[v] != bel[u]) {
                    ...
                    if (d == 0) {
                        if (OnFoundEdge(mat[v][u])) return true;
        ...
        for (int b = n + 1; b <= n_x; b++)
            if (bel[b] == b && col[b] == 1) tension(d, lab[b] / 2);
        for (int x = 1; x <= n_x; x++)
            if (bel[x] == x && slackv[x]) {
                if (col[x] == -1)
        ...
        q_n = 0;
        for (int v = 1; v <= n; v++)
            if (lab[v] == 0) return false;
        for (int x = 1; x <= n_x; x++)
            if (bel[x] == x && slackv[x] && bel[slackv[x]] != x && GetEdgeDelta(mat[slackv[x]][x]) == 0) {
                if (OnFoundEdge(mat[slackv[x]][x])) return true;
            }
        for (int b = n + 1; b <= n_x; b++)
            if (bel[b] == b && col[b] == 1 && lab[b] == 0) ExpandBlossom(b);
    }
    return false;
}
...
void GenGraph::CalcMaxWeightMatch() {
    ...
    for (int v = 1; v <= n; v++)
        for (int u = 1; u <= n; u++) blofrom[v][u] = v == u ? v : 0;

    int w_max = 0;
    for (int v = 1; v <= n; v++)
        for (int u = 1; u <= n; u++) relax(w_max, mat[v][u].w);
    for (int v = 1; v <= n; v++) lab[v] = w_max;

    while (DoMatching()) nMatches++;

    for (int v = 1; v <= n; v++)
        if (mate[v] && mate[v] < v) totWeight += mat[v][mate[v]].w;
}
...
GenGraph::GenGraph(int _n) {
    if (_n > 20000) {
        ...
        n = _n;
        for (int v = 1; v <= n; v++)
            for (int u = 1; u <= n; u++) mat[v][u] = Edge(v, u, 0);
    }
}

GenGraph::~GenGraph() {}

void GenGraph::AddEdge(int u, int v, int w) { mat[v][u].w = mat[u][v].w = w; }

s64 GenGraph::GetTotWeight() { return totWeight; }

vector<Match> GenGraph::GetMate() {
    vector<Match> matches;
    for (int i = 1; i <= n; i++) {
        if (mate[i] == 0) continue;
        if (i <= mate[i]) matches.push_back(Match(i, mate[i]));
    }
    return matches;
}